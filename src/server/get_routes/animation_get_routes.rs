...
pub(super) async fn make_routes(db_conn: &mut BoxedFilter<(SqlitePool,)>) -> BoxedFilter<(impl Reply,)> {
    ...
    let new_animation_page = warp::path("animation_new").map(|| {
        let body: String = fs::read_to_string("templates/animation/setupanimation.html").unwrap().parse().unwrap();
    ...
    let load_animation_page = warp::path("load_animation").map(|| {
        let body: String = fs::read_to_string("templates/animation/saved_animations.html").unwrap().parse().unwrap();
        let mut handlebars = Handlebars::new();
    ...
    let animation_menu_page = warp::path("animation").map(|| {
        let body: String = fs::read_to_string("templates/animation/animation_menu.html").unwrap().parse().unwrap();
        let mut handlebars = Handlebars::new();
        handlebars.register_template_string("tpl_2", body).unwrap();
        warp::reply::html(
            handlebars.render("tpl_2", &json!({})).unwrap()
...
async fn render_animation_page(guid: String, db_pool: Pool<Sqlite>) -> Result<Box<dyn Reply>, Rejection> {
    let body: String = fs::read_to_string("templates/animation/animation.html").unwrap().parse().unwrap();
